...
void TSearchTreeTable::balance(int& a, int& b, int p)
{
	if (b * p >= 0)
	{
		a = a + p;
...
void TSearchTreeTable::rotateLeft(TreeNode* a)
{
	TreeNode* b = a->pRight;
	b->pParent = a->pParent;
...
void TSearchTreeTable::rotateRight(TreeNode* a)
{
	TreeNode* b = a->pLeft;
	b->pParent = a->pParent;
	if (a->pParent != nullptr)
...
void TSearchTreeTable::rotateBigLeft(TreeNode* a)
{
	rotateRight(a->pRight);
	rotateLeft(a);
}

void TSearchTreeTable::rotateBigRight(TreeNode* a)
{
	rotateLeft(a->pLeft);
	rotateRight(a);
}

void TSearchTreeTable::changeBalance(TreeNode* p, int a)
{
...
void TSearchTreeTable::insert(const Data& _d)
{
	TreeNode* p = pRoot;
	TreeNode* pp = nullptr;
	int flag = 0;
...
void TSearchTreeTable::deleteByKey(const string& key)
{
...
void TSearchTreeTable::print()
{
	cout << "\tТаблица поисковое дерево " << endl;
	TreeNode* p = pRoot;
	if (pRoot == nullptr)
		cout << "Empty" << endl;
}

Data* TSearchTreeTable::find(const string key)
{
	TreeNode* p = pRoot;
	while (p != nullptr)
	{
