...
## Список объектов
 ...
 Для реализации программы будут использованы такие основные объекты, как:
  ...
  - среда исполнения внутреннего представления программы ("компилятор").

  
## Список основных алгоритмов

Для реализация навигации по иерархически-представленной программе необходимы:
  - переход к следующему звену программы;
  - переход на другой подуровень программы.

Для последовательного доступа ко всем элементам программы нужно реализовать:
  - обход программы.

Также понадобятся такие алгоритмы, как:
  - сборка мусора;
  - поиск ошибок.
...
**Строка** – значение объекта класса для строки текста программы, нижний уровень иерархической структуры.

**Текст** – это несколько предложений, связанных друг с другом по смыслу и грамматически. В рамках лабораторной работы в качестве примеров текстов рассматриваются тексты программ. Модель текста представлена иерархическим списком.
...
**Лексический анализатор (проверка синтаксиса, контроль ошибок, Lexical analysis)** - програмный комплекс, обеспечивающий проверку синтаксиса, удаление лишней информации из входного файла(пробелы).
Поддерживаемые операции:
 - .
  
**Сборка мусора** - реализация системы управления памятью для иерархически-представленной программы.

**Иерархический связный список** – это многосвязный список, в котором на каждое звено имеется ровно один указатель, а каждое звено содержит два указателя (один на следующее звено в том же уровне, другой на звено в нижерасположенном уровне). Использование единого типа звена уровня строки.
  Операции над списком:
  - переход к следующему звену программы;
  - переход на другой подуровень программы.

**Иерархический список** – это динамическая структура данных, состоящая из звеньев, каждое из которых содержит указатель на следующее звено того же уровня (pNext), указатель на подуровень (pDown) и какие-либо данные. В данном случае, данными будут являться строки (str).

<img src="https://user-images.githubusercontent.com/71322899/165158022-7d4f8fff-bb52-4db1-ab88-02b6288b2184.png" width="300" height="300">

pFirst – начало иерархического списка. Каждое такое звено будет хранить одну из строк текста. Если у звена нет подуровней, то pDown = NULL, аналогично с pNext.
Для перемещения по иерархическому списку будем использовать указатель pCurrent и стек указателей, который хранит путь от pFirst до pCurrent.

## Описание основных алгоритмов

**Обход программы**:

При начале обхода следует перейти от начала программы (корня дерева) до атомарного уровня по указателям нижерасположенных уровней, запоминая при этом все пройденные звенья в стеке. После обработки найденной строки, далее следует переходить по строкам того же уровня, до последней строки в этом уровне. После обхода уровня текущего уровня необходимо извлечь звено из стека и повторить всю выше приведенную последовательность действий. Обход текста будет завершен, когда стек пуст. Данная схема обхода представляет вариант TDN (top – down – next, вершина - нижний уровень – следующий элемент).

**Иерархический список**:
  - Переход к следующему звену программы
  - Переход на другой подуровень

**Сборка мусора**:

Алгоритм «сборки мусора» состоит из трех этапов. На первом этапе происходит обход программы и маркирование звеньев текста специальными символами (например, «&&&»). На втором этапе происходит обход и маркирование списка свободных звеньев. На третьем этапе происходит проход по всему непрерывному участку памяти как по непрерывному набору звеньев. Если звено промаркировано, то маркер снимается. В противном случае, найдено звено, на которое нет ссылок («мусор»), и это звено возвращается в список свободных звеньев.
